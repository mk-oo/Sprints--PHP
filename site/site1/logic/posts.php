...
function editMyPost($request, $oldpost_id, $image)
{
    $sql = "UPDATE posts 
            SET title = ?, content = ?, image = ?, publish_date = ?,created_at = ?, category_id = ?
            WHERE id =  ?";
...
function displayDataToBeEdited($postID){

    $sql = "SELECT p.*,c.* FROM posts p INNER JOIN categories c ON c.id=p.category_id WHERE p.id = $postID";
    return getRows($sql);
